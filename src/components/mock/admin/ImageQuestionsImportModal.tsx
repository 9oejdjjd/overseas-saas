/**
 * @file ImageQuestionsImportModal.tsx
 * @description المكون الإداري المتطور والجديد الخاص بتوليد واستيراد الأسئلة (بصور) خطوة بخطوة.
 * يدعم إضافة الأنماط المخصصة وعرضها لكل مهنة، ويشتمل على واجهة معالج رفع صور متسلسل (Wizard)
 * فسيحة وممتازة الاستخدام ومريحة للعين. خالي تماماً من الرموز التعبيرية.
 * 
...
export function ImageQuestionsImportModal({ professions, onSuccess }: Props) {
    const importHook = useQuestionsImport(professions, onSuccess, true);
    const {
        isOpen,
        setIsOpen,
        step,
    ...
    React.useEffect(() => {
        if (!isOpen || wizardQuestions.length === 0) return;
        ...
        const handleGlobalPaste = async (e: ClipboardEvent) => {
            ...
            if (imageFile) {
                e.preventDefault();
                await uploadWizardImageFile(imageFile);
            }
        };
    ...
    }, [isOpen, wizardQuestions.length, uploadWizardImageFile]);

    return (
        <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) resetForm(); }}>
            <DialogTrigger asChild>
                <Button className="gap-2 bg-gradient-to-r from-purple-650 to-indigo-650 hover:from-purple-700 hover:to-indigo-700 text-white shadow-md rounded-xl font-bold h-9 text-xs">
                    <ImageIcon className="h-4 w-4" />
                    توليد واستيراد أسئلة (بصور)
                </Button>
            </DialogTrigger>
            <DialogContent className="max-w-7xl max-h-[92vh] overflow-y-auto" dir="rtl">
                <DialogHeader className="border-b pb-3 mb-2">
                    <DialogTitle className="text-lg font-black text-slate-800 flex items-center gap-2">
                        <ImageIcon className="h-5 w-5 text-indigo-650" />
                        توليد وتغذية الأسئلة بصور
                    </DialogTitle>
                </DialogHeader>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-2">
                            ...
                            <div className="lg:col-span-5 space-y-4">
                                ...
                                <div className="bg-indigo-50/50 p-4 border border-indigo-150 rounded-2xl flex flex-col justify-between h-[180px]">
                                    <div className="space-y-1">
                                        <span className="font-black text-indigo-700 text-xs flex items-center gap-1.5">
                                            ...
                                            برومبت التوليد المهني المطور لأسئلة الصور:
                                        </span>
                                        <p className="text-[11px] text-indigo-900 leading-normal font-bold">
                                            سيوجه البرومبت الـ AI لصياغة أسئلة عملية وتحديد حقل requireImage كـ true وإضافة وصف مفصل للصورة المطلوبة باللغة العربية.
                                        </p>
                                    </div>
                                    <Button 
                                        onClick={copyPrompt} 
                                ...
                                <div className="space-y-3 bg-slate-50/50 p-4 border border-slate-150 rounded-2xl">
                                    ...
                                    <div className="space-y-1">
                                        <Label className="font-black text-slate-655 text-[10px]">آلية التحكم في تكرار الأسئلة:</Label>
                        ...
                        <div className="p-4 space-y-6">
                            <div className="flex items-center justify-between border-b pb-4">
                                <div className="text-right">
                                    ...
                                    <p className="text-xs text-gray-400 mt-1 font-bold">يرجى تأكيد توزيع وصعوبة الأسئلة للبدء في رفع صورها.</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-8 gap-2">
                                ...
                                <div className="p-2 rounded-xl bg-teal-50/50 border border-teal-100 text-center">
                                    <p className="text-[9px] text-teal-650 mb-0.5 font-bold">تطلب صور</p>
                                    <p className="text-lg font-black text-teal-700">{previewStats?.imageQuestionsCount || 0}</p>
                                </div>
                            </div>
                                    ...
                                    <div key={idx} className="bg-white border border-gray-100 p-4 rounded-xl shadow-sm text-right">
                                        <div className="flex justify-between items-start mb-2.5">
                                            ...
                                            <div className="flex gap-1.5">
                                                ...
                                                {(q.requireImage === true || q.requireImage === "true") && (
                                                    <Badge className="bg-teal-600 text-[9px] font-bold">يتطلب صورة</Badge>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {warnings && warnings.length > 0 && (
                                <div className="p-3.5 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl space-y-1.5 text-xs font-semibold text-right shadow-sm" dir="rtl">
                                    <div className="flex items-center gap-2 text-amber-900 border-b border-amber-200 pb-1 mb-1 font-bold">
                                        <AlertCircle className="h-4 w-4.5 text-amber-600 shrink-0" />
                                        <span>تنبيهات جودة الصياغة للأسئلة بالصور (غير مانعة للاستيراد):</span>
                                    </div>
                                    <ul className="list-disc list-inside space-y-1 pr-2">
                                        {warnings.map((warn, wIdx) => (
                                            <li key={wIdx} className="text-[11px] font-medium leading-relaxed list-item">
                                                {warn}
                                            </li>
                                        ))}
                                    </ul>
                            ...
                            <div className="flex justify-end gap-3 pt-4 border-t">
                                ...
                                <Button 
                                    ...
                                    disabled={isLoading}
                                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-black px-6 h-9 rounded-xl shadow-sm text-xs flex items-center gap-1.5"
                                >
                                    {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : `بدء معالج صور الأسئلة (${previewStats?.imageQuestionsCount || 0}) واكتمل الباقي`}
                                </Button>
                            </div>
                        </div>
                    )}

                    {/* خطوة معالج رفع صور الأسئلة المتسلسل (Image Upload Wizard) */}
                    {step === "wizard" && wizardQuestions.length > 0 && (
                        <div className="p-4 space-y-6 text-right" dir="rtl">
                            {/* شريط التقدم الفاخر */}
                            <div className="space-y-2">
                                <div className="flex justify-between items-center text-xs font-black text-indigo-700">
                                    <span>معالج رفع صور الأسئلة</span>
                                    <span>السؤال {currentWizardIndex + 1} من أصل {wizardQuestions.length}</span>
                                </div>
                                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden border">
                                    <div 
                                        className="bg-gradient-to-l from-indigo-500 to-purple-600 h-full transition-all duration-300"
                                        style={{ width: `${((currentWizardIndex + 1) / wizardQuestions.length) * 100}%` }}
                                    />
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* الجانب الأيمن: معاينة السؤال وتفاصيله */}
                                <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-150">
                                    <div className="flex justify-between items-center">
                                        <h4 className="font-black text-xs text-slate-500">معاينة السؤال الفني:</h4>
                                        <Badge className="bg-purple-600 text-[9px] font-bold">
                                            {wizardQuestions[currentWizardIndex].difficulty || "HARD"}
                                        </Badge>
                                    </div>
                                    <h3 className="font-extrabold text-sm text-slate-900 leading-relaxed">
                                        {wizardQuestions[currentWizardIndex].text}
                                    </h3>

                                    <div className="space-y-2 pt-2">
                                        {wizardQuestions[currentWizardIndex].options?.map((opt: any, idx: number) => (
                                            <div 
                                                key={idx} 
                                                className={`p-3 rounded-xl border text-xs flex items-center gap-2 ${
                                                    opt.isCorrect 
                                                        ? "bg-green-50 border-green-200 text-green-800 font-extrabold" 
                                                        : "bg-white border-slate-155 text-slate-500"
                                                }`}
                                            >
                                                <span className={`h-4.5 w-4.5 rounded-full flex items-center justify-center text-[10px] ${
                                                    opt.isCorrect ? "bg-green-600 text-white" : "bg-slate-100 text-slate-400"
                                                }`}>
                                                    {idx + 1}
                                                </span>
                                                <span>{opt.text}</span>
                                    ...
                                    {wizardQuestions[currentWizardIndex].explanation && (
                                        <div className="mt-3 p-3 bg-indigo-50/30 border border-indigo-100/50 rounded-xl text-[11px] text-indigo-950 leading-relaxed">
                                            <span className="font-black block mb-0.5 text-indigo-705">الشرح والتحليل الفني:</span>
                                            {wizardQuestions[currentWizardIndex].explanation}
                                        </div>
                                    )}
                                </div>

                                {/* الجانب الأيسر: رفع الصورة وتوجيه الذكاء الاصطناعي */}
                                <div className="space-y-4">
                                    {/* كادر وصف الصورة الموجه بالذكاء الاصطناعي */}
                                    <div className="bg-teal-50/60 border border-teal-100 p-4.5 rounded-2xl space-y-2">
                                        <span className="font-black text-teal-800 text-xs flex items-center gap-1.5">
                                            <ImageIcon className="h-4.5 w-4.5 text-teal-650" />
                                            الصورة المطلوبة للسؤال (وصف الـ AI):
                                        </span>
                                        <p className="text-xs text-teal-955 leading-relaxed font-extrabold bg-white p-3 rounded-xl border border-teal-200/55">
                                            {wizardQuestions[currentWizardIndex].imageDescription || "يرجى إرفاق صورة توضيحية تناسب نص السؤال الحالي."}
                                        </p>
                                    </div>

                                    {/* مساحة الرفع بالسحب والإفلات */}
                                    <div className="space-y-2">
                                        <Label className="font-bold text-gray-700 text-xs">ارفع صورة للمرفق المذكور:</Label>
                                        
                                        {wizardImageUrl ? (
                                            <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-white p-2 text-center">
                                                <img src={wizardImageUrl} alt="معاينة" className="w-full h-36 object-contain rounded-xl" />
                                                <Button 
                                                    variant="destructive" 
                                                    size="sm" 
                                                    className="absolute top-4 left-4 h-7 px-2.5 text-xs font-black shadow-sm"
                                                    onClick={() => setWizardImageUrl("")}
                                                >
                                                    حذف وإعادة الرفع
                                                </Button>
                                            </div>
                                        ) : (
                                            <div className="border-2 border-dashed border-indigo-200 hover:border-indigo-400 rounded-2xl p-8 text-center bg-indigo-50/5 hover:bg-indigo-50/10 transition-colors relative">
                                                {isUploadingWizardImage ? (
                                                    <div className="flex flex-col items-center justify-center gap-2 text-indigo-700 font-bold">
                                                    ...
                                                    <div className="flex flex-col items-center justify-center gap-2 text-slate-500">
                                                        ...
                                                        <span className="text-xs font-extrabold text-indigo-950">اسحب الصورة هنا، اضغط للتصفح، أو ألصقها مباشرة بـ Ctrl+V</span>
                                                        <input 
                                                            type="file" 
                                                            accept="image/*"
                                                            onChange={handleWizardImageUpload}
                            ...
                            {error && (
                                <div className="p-3.5 bg-red-50 text-red-700 border border-red-155 rounded-xl flex gap-2 text-xs font-bold shadow-sm">
                                    <AlertCircle className="h-4.5 w-4.5 shrink-0 text-red-500" />
                                    <p>{error}</p>
                                </div>
                            )}
                            ...
                            <div className="flex justify-between items-center pt-4 border-t">
                                <Button 
                                    variant="ghost" 
                                    onClick={() => saveWizardQuestion(true)} 
                                    disabled={isLoading}
                                    className="text-slate-400 hover:text-slate-655 text-xs font-bold"
                                >
                                    تجاوز هذا السؤال بدون صورة ➡️
                                </Button>
                                
                                <div className="flex gap-2">
                                    <Button variant="outline" onClick={resetForm} className="h-9 text-xs font-bold rounded-xl">إلغاء</Button>
                                    <Button 
                                        onClick={() => saveWizardQuestion(false)} 
                                        disabled={isLoading || isUploadingWizardImage || !wizardImageUrl}
                                        className="bg-indigo-650 hover:bg-indigo-700 text-white font-black px-6 h-9 rounded-xl shadow-md text-xs flex items-center gap-1.5"
                                    >
                                        {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : (
                                            <>
                                                <span>حفظ ورفع الصورة والتالي</span>
                    ...
                    {step === "report" && report && (
                        <div className="p-8 text-center space-y-6" dir="rtl">
                            ...
                            <div className="space-y-1.5">
                                <h3 className="text-xl font-black text-gray-900">اكتمل استيراد وتوليد الأسئلة بنجاح!</h3>
                                <p className="text-xs text-gray-400 font-bold">تم إدخال وتغذية بنك الأسئلة للمهنة المحددة بالخيارات والشروحات والصور بنجاح.</p>
                            </div>
