...
import { X, Plus, Save, MapPin, Settings } from "lucide-react";
...
export function LocationsList() {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5 text-gray-700" /> مراكز الاختبار</CardTitle>
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 space-y-4">
                    <div className="flex gap-4 items-end">
                        <div className="space-y-2 flex-1">
                            <label className="text-sm font-medium">اسم المدينة (مثال: مأرب)</label>
                            <Input value={newName} onChange={e => setNewName(e.target.value)} placeholder="اسم المدينة" className="bg-white" />
                        </div>
                        <div className="space-y-2 w-32">
                            <label className="text-sm font-medium">الكود (اختياري)</label>
                            <Input value={newCode} onChange={e => setNewCode(e.target.value)} placeholder="MRB" className="bg-white" />
                        </div>
                ...
                <div className="border rounded-md overflow-hidden">
                    <table className="w-full text-sm text-right">
                        ...
                        <tbody className="divide-y">
                            ...
                            {locations.length === 0 && !loading && <tr><td colSpan={5} className="p-4 text-center text-gray-500">لا توجد مواقع</td></tr>}
                        </tbody>
                    </table>
                </div>
            </CardContent>
